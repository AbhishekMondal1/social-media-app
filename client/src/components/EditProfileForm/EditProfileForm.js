...
const EditProfileForm = () => {
  ...
  const handleFormSubmit = async (e) => {
    ...
    if (username_regex.test(details.username) === false) {
      setDetailsError((prev) => (
        {
          ...prev,
          usernameerr: "Username should be 1-30 characters and should not include special characters",
        }
      ))
      inputError = true;
    }
  ...
  useEffect(() => {
    if (image) {
      ...
      axios.post("https://api.cloudinary.com/v1_1/digimode/image/upload", data)
        .then((res) => res.data)
        .then((res) => {
          axios.put("/updatepic", {
            pic: res.url,
      ...
      <article className="main-article">
        <div className="avatar-wrapper">
          ...
          <div className="avatar-btn">
            ...
            <input className="inputfile" ref={inputFileRef} type="file" name="file" id="file" accept=".png, .jpg, .jpeg"
        ...
        <div className="form-wrapper">
          <form onSubmit={handleFormSubmit}>
            <div className="flexrow">
              ...
              <input className="input" type="text" name="name" id="fname" placeholder="name"
                value={details.name} onChange={handleFormInputChange} onFocus={handleFormInputFocus} />
            </div>
            {detailsError.nameerr &&
              <Tooltip title={detailsError.nameerr} />
            }
            <div className="flexrow">
              <label htmlFor="fusername">
                Username
              </label>
              <input className="input" type="text" name="username" id="fusername" placeholder="username"
                value={details.username} onChange={handleFormInputChange} onFocus={handleFormInputFocus} />
            </div>
            ...
            <div className="flexrow">
              ...
              <textarea className="textarea" type="text" name="bio" id="fbio" placeholder="bio"
                value={details.bio} onChange={handleFormInputChange} onFocus={handleFormInputFocus} />
            </div>
            {detailsError.bioerr &&
              <Tooltip title={detailsError.bioerr} />
            }
            <div className="flexrow">
              <label htmlFor="femail">
                Email
              </label>
              <input className="input" type="text" name="email" id="femail" placeholder="email"
                value={details.email} onChange={handleFormInputChange} onFocus={handleFormInputFocus} />
            </div>
            {detailsError.emailerr &&
              <Tooltip title={detailsError.emailerr} />
            }
